export class VDF {
	...
	static parse(str: string): any {
		...
		const next = (lookAhead: boolean = false): string => {
			...
			if (str[j] == '"') {
				...
				while (str[j] != '"' && j < str.length) {
					if (str[j] == '\n') {
						throw new Error(`Unexpected EOL at position ${j} (line ${line}, position ${character})! Are you missing a closing "?`);
					}
					currentToken += str[j];
					j++;
				}
				...
				while (!whiteSpaceIgnore.includes(str[j]) && j < str.length - 1) {
					if (str[j] == '"') {
						throw new Error(`Unexpected '"' at position ${j} (line ${line}, position ${character})! Are you missing terminating whitespace?`);
					}
					currentToken += str[j];
					j++;
				}
		...
		const parseObject = (): { [key: string]: any } => {
			...
			while (currentToken != "}" && nextToken != "EOF") {
				let lookahead: string = next(true)
	...
	static stringify(obj: any, indentation: "Tabs" | "Spaces" = "Tabs", newLine: "CRLF" | "LF" = "CRLF"): string {
		...
		const getIndentation: (level: number) => string = tabIndentation ? (level: number) => tab.repeat(level) : (level: number) => space.repeat(level * 4)
		const getWhitespace: (longest: number, current: number) => string = tabIndentation
			? (longest: number, current: number) => tab.repeat(((longest + 2) / 4 - (current + 2) / 4) + 2)
