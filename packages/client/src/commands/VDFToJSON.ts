import { VDF } from "vdf"
import vscode from "vscode"

export function VDFToJSON(editor: vscode.TextEditor, edit: vscode.TextEditorEdit): void {
	const { document } = editor
	const indentation = !editor.options.insertSpaces ? "\t" : "    "
	if (!editor.selection.isEmpty) {
		edit.replace(editor.selection, JSON.stringify(VDF.parse(document.getText(editor.selection)), null, indentation))
	}
	else {
		edit.replace(new vscode.Range(0, 0, document.lineCount, 0), JSON.stringify(VDF.parse(document.getText()), null, indentation))
		vscode.languages.setTextDocumentLanguage(document, "json")
	}
}
